...
@implementation MSQImagePickersVC
...
- (void)initView {
    ...
    if (oriWidth >= oriHeight) {
        ...
        oriHeight = self.cropFrame.size.width;
...
- (void)initControlBtn {
    UIButton *cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 100.0f, 100, 100)];
    ...
    UIButton *confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, self.view.frame.size.height - 100.0f, 100, 100)];
    confirmBtn.backgroundColor = [UIColor clearColor];
    confirmBtn.titleLabel.textColor = [UIColor whiteColor];
    [confirmBtn setTitle:@"剪切" forState:UIControlStateNormal];
    ...
    UIButton *originalBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 50.0f, self.view.frame.size.height - 100.0f, 100, 100)];
    originalBtn.backgroundColor = [UIColor clearColor];
    originalBtn.titleLabel.textColor = [UIColor whiteColor];
    ...
    //    [confirmBtn.titleLabel setNumberOfLines:0];
    //    [confirmBtn setTitleEdgeInsets:UIEdgeInsetsMake(5.0f, 5.0f, 5.0f, 5.0f)];
    [originalBtn addTarget:self action:@selector(original:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:originalBtn];
}

- (void)cancel:(id)sender {
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(MSQImageClipDelegate)]) {
...
- (void)original:(id)sender {
    
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(MSQImageClipDelegate)]) {
        [self.delegate imageCropper:self didFinished:self.originalImage];
    }
}
